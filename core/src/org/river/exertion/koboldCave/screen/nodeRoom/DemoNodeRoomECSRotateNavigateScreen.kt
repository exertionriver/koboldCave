...
class DemoNodeRoomECSRotateNavigateScreen(private val batch: Batch,
    ...
    var curAngle = playerCharacter[ActionMoveComponent.mapper]!!.currentAngle
    var prevAngle = playerCharacter[ActionMoveComponent.mapper]!!.currentAngle

    override fun render(delta: Float) {

        batch.projectionMatrix = camera.combined
        camera.update()
//        camera.rotate(Vector3.Z, curAngle - prevAngle)
//        camera.position.lerp(Vector3(playerCharacter[ActionMoveComponent.mapper]!!.currentPosition.x, playerCharacter[ActionMoveComponent.mapper]!!.currentPosition.y, 0f), 0.5f)

//        println("1/delta : ${1/delta}")
        val losMap = nodeRoomMesh.renderWallsAndPathLos(playerCharacter[ActionMoveComponent.mapper]!!.currentPosition, playerCharacter[ActionMoveComponent.mapper]!!.currentAngle, visualRadius)
...
//            println("PC position: ${playerCharacter.getEntityComponent().currentPosition}")
        ...
        engine.update(delta)
//        prevAngle = curAngle.toInt().toFloat()
//        curAngle = playerCharacter[ActionMoveComponent.mapper]!!.currentAngle.toInt().toFloat()
//        println("prev, cur: $prevAngle, $curAngle")
        }
    }
