...
/*
    ...
    fun getStateAction(stateActionUuid: UUID) : ActionInstance = if (actionEntries[stateActionUuid] != null) actionEntries[stateActionUuid]!! else ActionInstance.StateActionNone
    ...
    fun cycleState(stateActionUuid : UUID) {
        when (getActionState(stateActionUuid) ) {
            ActionState.ActionQueue -> actionEntries[stateActionUuid] = ActionInstance(copyStateAction = actionEntries[stateActionUuid]!!, updActionState = ActionState.ActionPrepare, updTimer = Timer())
            ActionState.ActionPrepare -> actionEntries[stateActionUuid] = ActionInstance(copyStateAction = actionEntries[stateActionUuid]!!, updActionState = ActionState.ActionExecute, updTimer = Timer())
            ActionState.ActionExecute -> actionEntries[stateActionUuid] = ActionInstance(copyStateAction = actionEntries[stateActionUuid]!!, updActionState = ActionState.ActionRecover, updTimer = Timer())
            ActionState.ActionRecover -> actionEntries[stateActionUuid] = ActionInstance(copyStateAction = actionEntries[stateActionUuid]!!, updActionState = ActionState.ActionQueue, updTimer = Timer())
            else -> actionEntries[stateActionUuid] = ActionInstance(copyStateAction = actionEntries[stateActionUuid]!!, updActionState = ActionState.ActionQueue)
        }
    }
    ...
    fun initAction(action: Action, actionPriority: ActionPriority, actionParamList : ParamList? = null, condition : Condition? = Always, conditionParamList : ParamList? = null) {
        if (numActionsRunning(action) < action.maxParallel)  {
            ...
            when (actionPriority) {
                ActionPriority.BaseAction -> actionEntries[newPlexActionUuid] = ActionInstance(
                ...
                else -> actionEntries[newPlexActionUuid] = ActionInstance(action, action.plexSlotsRequired, ActionState.ActionQueue, actionPriority, actionParamList)
            }
...
//                println("actionExec: ${getAction(stateActionUuid)}, ${getCondition(stateActionUuid)}, ${getConditionParamList(stateActionUuid)}")
                ...
                if ( getAction(stateActionUuid) == Idle) {
                    actionEntries[stateActionUuid] = ActionInstance(copyStateAction = getStateAction(stateActionUuid)
    ...
    fun interruptAction(stateActionUuid : UUID) {
        when (getActionState(stateActionUuid)) {
            ...
            ActionState.ActionRecover -> actionEntries[stateActionUuid] = ActionInstance(copyStateAction = getStateAction(stateActionUuid), updActionState = ActionState.ActionRecover, updTimer = Timer())    //restart recover
    ...
    fun preemptAction(stateActionUuid : UUID) {
        actionEntries[stateActionUuid] = ActionInstance(copyStateAction = getStateAction(stateActionUuid), updActionState = ActionState.ActionQueue, updTimer = Timer())
    }
...
//    println("interrupt($slotsToInterrupt, $maxPlexSize)")
        ...
        val interruptables = actionEntries.filterValues { ActionState.Interruptable.contains(it.actionState) }.toList().sortedWith (compareByDescending <Pair<UUID, ActionInstance>> { it.second.actionPriority }.thenBy { it.second.timer.getMillisecondsElapsed() })
...
//    println("filled slots to preempt:${filledSlotsToPreempt}")
        ...
        }.toList().sortedWith (compareByDescending <Pair<UUID, ActionInstance>> { it.second.actionPriority }.thenBy { it.second.timer.getMillisecondsElapsed() })
...
//    preemptables.forEach { println("preemptable: $it") }
  ...
  //          println("${preemptable.first} PREEMPTED!" )
    ...
    fun stateString() : List<String> {
        ...
        actionEntries.toList().sortedWith (compareBy<Pair<UUID, ActionInstance>> { it.second.actionPriority }.thenByDescending { it.second.timer.getMillisecondsElapsed() }).forEach{ returnState.add("${it.first}: ${it.second}") }
