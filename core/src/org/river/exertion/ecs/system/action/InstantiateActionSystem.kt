...
class InstantiateActionSystem : IteratingSystem(allOf(InstantiateActionComponent::class).get()) {
    ...
    override fun processEntity(entity: Entity, deltaTime: Float) {
        ...
        if ( ILocation.has(entity) && MomentComponent.has(entity) && ILocation.getFor(entity)!!.stateMachine.currentState == ActionState.SOME ) {
            ...
            if ( engine.entities.filter { CharacterKobold.has(it) }.count() < 2) {
                CharacterKobold.instantiate(this.engine as PooledEngine, entity[InstantiateActionComponent.mapper]!!.stage, location = entity)
