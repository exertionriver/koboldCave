...
//usually, it is better to use Quaternion
                ...
                Gdx.input.isKeyJustPressed(Input.Keys.R) -> { camera.rotateAround(Vector3(refVector.x, refVector.y, refVector.z), Vector3(1f, 0f, 0f), 15f) }
                Gdx.input.isKeyJustPressed(Input.Keys.F) -> { camera.rotateAround(Vector3(refVector.x, refVector.y, refVector.z), Vector3(0f, 1f, 0f), 15f) }
                Gdx.input.isKeyJustPressed(Input.Keys.V) -> { camera.rotateAround(Vector3(refVector.x, refVector.y, refVector.z), Vector3(0f, 0f, 1f), 15f) }
//                else -> //do nothing
        }
    }
}
